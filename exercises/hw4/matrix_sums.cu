...
__global__ void row_sums(const float *A, float *sums, size_t ds) {
  ...
  int idx =
      threadIdx.x +
      blockIdx.x *
          blockDim.x; // create typical 1D thread index from built-in variables
  if (idx < ds) {
    float sum = 0.0f;
    for (size_t i = 0; i < ds; i++)
      sum += A[idx * ds + i]; // write a for loop that will cause the thread to
                              // iterate across a row, keeping a running sum,
  ...
  if (idx < ds) {
    ...
    for (size_t i = 0; i < ds; i++)
      sum += A[i * ds + idx]; // write a for loop that will cause the thread to
                              // iterate down a column, keeping a running sum,
                              // and write the result to sums
    sums[idx] = sum;
  }
}
