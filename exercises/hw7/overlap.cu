...
int main() {
  ...
  const auto chunkSize = ds / chunks;
  for (int i = 0; i < chunks; i++) { // depth-first launch
    cudaMemcpyAsync(d_x + i * chunkSize, h_x + i * chunkSize,
                    chunkSize * sizeof(ft), cudaMemcpyHostToDevice,
                    streams[i % num_streams]);
    gaussian_pdf<<<(chunkSize + 255) / 256, 256, 0, streams[i % num_streams]>>>(
        d_x + i * chunkSize, d_y + i * chunkSize, 0.0, 1.0, chunkSize);
    cudaMemcpyAsync(h_y + i * chunkSize, d_y + i * chunkSize,
                    chunkSize * sizeof(ft), cudaMemcpyDeviceToHost,
                    streams[i % num_streams]);
  }
  cudaDeviceSynchronize();
